...
import com.lon.fiber.core.SignalAmpl;
import com.lon.fiber.core.SignalAmplPoint;
...
public class SignalDetailView extends View {
	...
	static final int sampleRate = 6250;
	...
	int centerIndex = 6250/2; // 光标对应的数据的索引
	int xCursorIndex = 300; // X光标的位置

	float PixelPerData = 2; // 每个数据占用的像素数

	private float upper = 30; // 上限
	private float lower = -1;// 下限
	...
	private void drawSignalData(Canvas canvas) {
		...
		if(signalData.length<=1) return;
		screenWidth = this.getWidth();
		screenHeight = this.getHeight();
		int displayWidth = screenWidth - LeftMargin;

		if (pointsList == null || pointsList.length / 8 < displayWidth) {
			pointsList = new float[displayWidth * 8+100];
		}

		if(PixelPerData<0)
		{
			PixelPerData=displayWidth*1f/6250;
